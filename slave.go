...
func newSlaveCollector(httpClients []*httpClient) prometheus.Collector {
	...
	metrics := map[prometheus.Collector]func(metricMap, metricInfoMap, prometheus.Collector) error{
		...
		gauge("slave", "cpus", "Current CPU resources in cluster.", "type", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "cpus_revocable", "Current revocable CPU resources in cluster.", "type", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "mem", "Current memory resources in cluster.", "type", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "mem_revocable", "Current revocable memory resources in cluster.", "type", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "gpus", "Current GPU resources in cluster.", "type", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "gpus_revocable", "Current revocable GPUS resources in cluster.", "type", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "disk", "Current disk resources in cluster.", "type", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "disk_revocable", "Current disk resources in cluster.", "type", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "registered", "1 if slave is registered with master, 0 if not.", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			registered, ok := m["slave/registered"]
			if !ok {
				log.WithField("metric", "slave/registered").Warn(LogErrNotFoundInMap)
			}
			c.(*prometheus.GaugeVec).WithLabelValues(mi["hostname"]).Set(registered)
			return nil
		},
		gauge("slave", "uptime_seconds", "Number of seconds the slave process is running.", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		counter("slave",
			"recovery_errors",
			"Total number of recovery errors", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			errors, ok := m["slave/recovery_errors"]
			if !ok {
				log.WithField("metric", "slave/recovery_errors").Warn(LogErrNotFoundInMap)
			}
			c.(*settableCounterVec).Set(errors, mi["hostname"])
			return nil
		},
		gauge("slave", "recovery_time_secs", "Agent recovery time in seconds", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			age, ok := m["slave/recovery_time_secs"]
			if !ok {
				log.WithField("metric", "slave/recovery_time_secs").Warn(LogErrNotFoundInMap)
			}
			c.(*prometheus.GaugeVec).WithLabelValues(mi["hostname"]).Set(age)
			return nil
		},
		gauge("slave", "executor_directory_max_allowed_age_secs", "Max allowed age of the executor directory", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "executor_state", "Current number of executors by state.", "state", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			...
			c.(*prometheus.GaugeVec).WithLabelValues("registering", mi["hostname"]).Set(registering)
			c.(*prometheus.GaugeVec).WithLabelValues("running", mi["hostname"]).Set(running)
			c.(*prometheus.GaugeVec).WithLabelValues("terminating", mi["hostname"]).Set(terminating)
			return nil
		},
		gauge("slave", "frameworks_active", "Current number of active frameworks", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			active, ok := m["slave/frameworks_active"]
			if !ok {
				log.WithField("metric", "slave/frameworks_active").Warn(LogErrNotFoundInMap)
			}
			c.(*prometheus.GaugeVec).WithLabelValues(mi["hostname"]).Set(active)
			return nil
		},
		counter("slave",
			"executors_terminated",
			"Total number of executor terminations.", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			terminated, ok := m["slave/executors_terminated"]
			if !ok {
				log.WithField("metric", "slave/executors_terminated").Warn(LogErrNotFoundInMap)
			}
			c.(*settableCounterVec).Set(terminated, mi["hostname"])
			return nil
		},
		counter("slave",
			"executors_preempted",
			"Total number of executor preemptions.", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			preempted, ok := m["slave/executors_preempted"]
			if !ok {
				log.WithField("metric", "slave/executors_preempted").Warn(LogErrNotFoundInMap)
			}
			c.(*settableCounterVec).Set(preempted, mi["hostname"])
			return nil
		},

		// Slave stats about tasks
		counter("slave", "task_states_exit_total", "Total number of tasks processed by exit state.", "state", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		counter("slave", "task_states_current", "Current number of tasks by state.", "state", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			...
			c.(*settableCounterVec).Set(killing, "killing", mi["hostname"])
			c.(*settableCounterVec).Set(running, "running", mi["hostname"])
			c.(*settableCounterVec).Set(staging, "staging", mi["hostname"])
			c.(*settableCounterVec).Set(starting, "starting", mi["hostname"])

			return nil
		},

		counter("slave", "task_state_counts_by_source_reason", "Number of task states by source and reason", "state", "source", "reason", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		counter("slave", "messages_outcomes_total",
			...
			"type", "outcome", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			...
			c.(*settableCounterVec).Set(frameworkMessagesValid, "framework", "valid", mi["hostname"])
			c.(*settableCounterVec).Set(frameworkMessagesInvalid, "framework", "invalid", mi["hostname"])
			c.(*settableCounterVec).Set(statusUpdateValid, "status", "valid", mi["hostname"])
			c.(*settableCounterVec).Set(statusUpdateInvalid, "status", "invalid", mi["hostname"])

			return nil
		},

		// GC information
		gauge("slave", "gc_path_removals_pending", "Number of sandbox paths that are currently pending agent garbage collection", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			pending, ok := m["gc/path_removals_pending"]
			if !ok {
				log.WithField("metric", "gc/path_removals_pending").Warn(LogErrNotFoundInMap)
			}
			c.(*prometheus.GaugeVec).WithLabelValues(mi["hostname"]).Set(pending)
			return nil
		},
		counter("slave", "gc_path_removals_outcome",
			"Number of sandbox paths the agent removed",
			"outcome", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			...
			c.(*settableCounterVec).Set(succeeded, "success", mi["hostname"])
			c.(*settableCounterVec).Set(failed, "failed", mi["hostname"])
		...
		counter("slave",
			...
			"Total number of container launch errors", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		counter("slave",
			...
			"Number of containers changing root filesystem", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
			...
			c.(*settableCounterVec).Set(newRootfs, mi["hostname"])
			return nil
		},
		counter("slave",
			"containerizer_provisioner_bind_remove_rootfs_errors",
			"Number of errors from the containerizer attempting to bind the rootfs", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		counter("slave",
			...
			"Number of errors from the containerizer attempting to remove a container", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		counter("slave",
			...
			"Number of containers destroyed due to launch errors", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
		...
		gauge("slave", "containerizer_fetcher_cache_size", "Containerizer fetcher cache sizes in bytes", "type", "hostname"): func(m metricMap, mi metricInfoMap, c prometheus.Collector) error {
